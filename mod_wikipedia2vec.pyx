"""
NOTE: This code is forked from https://github.com/wikipedia2vec/wikipedia2vec/blob/master/wikipedia2vec/wikipedia2vec.pyx.
My contribution here is modifying the loss function to incorporate my categorical code.

To that end, anything that you see involving `cat_graph` or `cat_graph_obj` is me. 
I also modified the `train_pair` function to accept weights and to support the category
embedding. Everything else is courtesy of the Wikipedia2vec team.
"""
...
cdef class Wikipedia2Vec:
    ...
    def train(self, dump_db, cat_graph, link_graph, mention_db, tokenizer, sentence_detector, pool_size,
        ...
        cat_graph_obj = None
        if cat_graph is not None:
            # Not sparse representation for now
            cat_graph_obj = cat_graph
...
def init_worker(dump_db_, dictionary_obj, cat_graph_obj, link_graph_obj, mention_db_obj, tokenizer_,
    ...
    if cat_graph_obj is not None:
        # Switch to data_test for testing
        cat_graph = np.loadtxt('data_train.csv', delimiter=',')
    else:
        cat_graph = None
...
def train_page(tuple arg):
    ...
    if link_graph is not None:
        ...
        for i in range(start, start + params.entities_per_page):
            ...
            for j in range(len(neighbors)):
                _train_pair(entity, neighbors[j], alpha_, params.negative, entity_neg_table, 1)

    # Train using category graph
    ...
    if cat_graph is not None:
        ...
        for i in range(start, start + params.entities_per_page):
            ...
            train_idx = dictionary.get_entity_index(article)
            for j in range(cat_graph[i]):
                if cat_graph[i][j] != 0:
                    _train_pair(train_idx, j, alpha_, params.negative, entity_neg_table)
    ...
    for paragraph in paragraphs:
        ...
        for i in range(len(words)):
            ...
            for j in range(start, end):
                ...
                _train_pair(word, word2, alpha_, params.negative, word_neg_table, 1)
        ...
        for (entity, ent_start, ent_end) in target_links:
            ...
            for j in range(start, end):
                ...
                _train_pair(entity, word2, alpha_, params.negative, word_neg_table, 1)
                _train_pair(word2, entity, alpha_, params.negative, entity_neg_table, 1)
...
cdef inline void _train_pair(int32_t index1, int32_t index2, float32_t alpha, int32_t negative,
                             int32_t [:] neg_table, float32_t weight) nogil:
    ...
    cdef float32_t weightf = <float32_t>weight
    ...
    for d in range(negative + 1):
        ...
        temp_dot = <float32_t>(blas.sdot(&dim_size, &weightf, &one, &syn0[index, 0], &one))
        f_dot = <float32_t>(blas.sdot(&dim_size, temp_dot, &one, &syn1[index, 0], &one))
